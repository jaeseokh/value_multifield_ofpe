...
vip_outdir <- here::here("Results","Figures","stage1_ensemble")
...
labs(x=NULL, y="Importance", title=paste0("RF VI — ", id)) +
theme_minimal(base_size = 11)
} else if (model == "XGB" && all(c("Feature","Gain") %in% names(df))) {
ggplot(df, aes(x = reorder(Feature, Gain), y = Gain)) +
geom_col(fill = "darkorange") + coord_flip() +
labs(x=NULL, y="Gain", title=paste0("XGB VI — ", id)) +
theme_minimal(base_size = 11)
} else NULL
}
...
tree_outdir <- here::here("Results","Figures","stage1_ensemble")
...
curve_outdir <- here::here("Results","Figures","stage1_ensemble")
...
title = paste0("Yield–N curves — ", ff, " (3 trials)"),
color = "Model") +
...
readr::write_csv(eonr_all, here::here("Figures","EONR_3trial_all.csv"))
